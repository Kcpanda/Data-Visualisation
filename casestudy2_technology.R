...
library(dplyr)
datatable%>%filter(datatable$the_list.Industry=="Technology")->technology
View(technology)
library(ggplot2)
#converting to numeric
technology$the_list.Brand.Revenue<-as.numeric(technology$the_list.Brand.Revenue)
technology$the_list.Company.Advertising<-as.numeric(technology$the_list.Company.Advertising)
technology$the_list.Brand.Value<-as.numeric(technology$the_list.Brand.Value)
#aesthetic
tech<-ggplot(technology,aes(x=technology$the_list.Company.Advertising,y=technology$the_list.Brand.Revenue,label=the_list.Brand))
#labelling
s<-tech+xlab("Comapany Advertising in Billions of $") + ylab("Brand Revenue in Billions of $")
#geometry
p<-s+geom_point(aes(size=the_list.Brand.Value,color=the_list.Brand))+guides(color=F)
p<-p+scale_size_continuous(name="Brand Value $(Billions)",breaks=c(30,60,100),labels=c(30,60,100))
#putting text on map
p+geom_text(vjust=0,nudge_y = -5,aes(color=the_list.Brand,size=the_list.Brand.Value))+theme_bw()+theme(legend.key = element_rect(fill = "lightblue",color = "black"),plot.title = element_text(size = 40,hjust = 0.5) )+ggtitle("Technology")
